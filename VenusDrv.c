...
static ssize_t hfdu04_read(struct file *file, char *buf, size_t count, loff_t * ppos)
{
   ...
   if ( dev->bcdDevice >= 0x4000)
   {
      ...
      while (remain_count > 0) 
      {
         ...
         // 읽은 데이터를 copy_to_user 함수를 이용하여 어플리케이션으로 넘긴다.
   ...
   else if ( dev->bcdDevice < 0x2000)
   {
      ...
      while (count > 0) 
      {
         // 01 버전에서 사용하던 ISO 방식을 이용한 데이터 전송 방식.
         // ISO 방식의 경우 hfdu04_startgrabbingdata 함수에 있는 hfdu04_allocbuffers를 이용하여, URB 초기화 하고,
         // 초기화한 URB를 usb_submit_urb 함수를 이용하여 URB를 USB 코어단으로 넘겨주게 된다.
         // usb_submit_urb 가 정상적으로 끝나게 되면, URB를 초기화 할때 등록한 callback 함수 ( hfdu04_isocomplete )를 호출하게 된다.
         // 이 hfdu04_isocomplete 함수에서 데이터를 copy_to_user 함수를 이용하여 어플리케이션으로 넘긴다.
...
static ssize_t hfdu04_write(struct file *file, const char *buf, size_t count, loff_t * ppos)
{
   ...
   if ( dev->bcdDevice >= 0x4000)
   {
      ...
      while (remain_count > 0) 
      {
         ...
         // 쓸 데이터를 copy_from_user 함수를 이용하여 kernel buffer로 옮긴다.
   ...
   {
     ret = 0; // HFDU01 은 bulk write 를 지원하지 않는다.
   }
                        ...
                        const struct usb_device_id *id)
{
   ...
   if ( (udev->descriptor.idVendor != USB_HFDU04_VENDOR_ID)
         && (udev->descriptor.idProduct != USB_HFDU04_PRODUCT_ID)) 
   {
#if LINUX_VERSION_CODE > KERNEL_VERSION(3,3,0)
      ...
      else
      {
            ...
            ((endpoint->bmAttributes & USB_ENDPOINT_XFERTYPE_MASK) == USB_ENDPOINT_XFER_BULK) )
         {
            ...
            if (buffer_size == 64) // for full speed
            {
               ...
               {
                  //dev->maxbulktransfersize = 384; // 384 = 64*6 (1ms 당 6번의 transaction을 시도함)
